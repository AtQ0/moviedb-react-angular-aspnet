...
public sealed class MovieService(TmdbClient tmdb)
{
    ...
    {
        var page = await tmdb.GetMoviesAsync(type, genreId, query, ct);
        return page.Results.Select(MapMovie).ToList();
    }

    public async Task<IReadOnlyList<GenreDto>> GetGenresAsync(CancellationToken ct)
    {
        var res = await tmdb.GetGenresAsync(ct);
        return res.Genres.Select(g => new GenreDto(g.Id, g.Name)).ToList();
    }
    ...
    private static MovieDto MapMovie(TmdbMovieResult r) => new(
        ...
        PosterUrl: string.IsNullOrWhiteSpace(r.PosterPath) ? null : $"{ImageBase}{r.PosterPath}",
        BackdropUrl: string.IsNullOrWhiteSpace(r.BackdropPath) ? null : $"{ImageBase}{r.BackdropPath}",
